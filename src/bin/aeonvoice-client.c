/* Client program to connect to AeonVoice running in the daemon mode */
...
static void add_path(char **path, const char *str)
{
  ...
  if (!str || !str[0])
    {
      fprintf(stderr, "Invalid empty path component\n");
      exit(1);
    }
  len1 = *path ? strlen(*path) : 0;
  len2 = strlen(str);
  new_path = realloc(*path, len1 + len2 + 2);
  if (!new_path)
    {
      perror("realloc");
      exit(1);
...
int main(int argc, char **argv)
{
  ...
  if (daemon_dir)
  ...
  else
    {
      ...
      if (!home || !home[0])
        {
          fprintf(stderr, "HOME is not set; unable to determine daemon directory\n");
          exit(1);
        }
      add_path(&socket_name, home);
      add_path(&socket_name, ".aeonvoice");
    }
