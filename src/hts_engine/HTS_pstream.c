...
static void HTS_PStream_calc_wuw_and_wum(HTS_PStream * pst, size_t m)
{
   ...
   for (t = 0; t < pst->length; t++) {
      ...
      /* calc WUW & WUM */
      for (i = 0; i < pst->win_size; i++) /* MAR i=0 for param, i=1 for delta, i=2 for delta2 */
         for (shift = pst->win_l_width[i]; shift <= pst->win_r_width[i]; shift++) /* MAR i=0, shift is 0, for others, shift -1,0,1 */
            if (((int) t + shift >= 0) && ((int) t + shift < pst->length) && (pst->win_coefficient[i][-shift] != 0.0)) {
               wu = pst->win_coefficient[i][-shift] * pst->sm.ivar[t + shift][i * pst->vector_length + m];
				/* the wum for t collects the mean for every vi, and shift weighted by the wu */
...
HTS_Boolean HTS_PStreamSet_create(HTS_PStreamSet * pss, HTS_SStreamSet * sss, double *msd_threshold, double *gv_weight)
{
   ...
   for (i = 0; i < pss->nstream; i++) {
      ...
      /* copy pdfs */
			/* previous frames */
   		HTS_Boolean prev_bound;
   		HTS_Boolean prev_first_bound;
   		HTS_Boolean prev_msd_bound;
			size_t prev_frames_remaining = sss->sstream[i].num_prev_frames;
/*			prev_frames_remaining = 0;  */
            ...
            for (j = 0; j < HTS_SStreamSet_get_duration(sss, state); j++) {
               if (pst->msd_flag[frame] == TRUE) {
                  ...
                  for (k = 0; k < pst->win_size; k++) {
                     not_bound = TRUE;
   									 prev_bound = FALSE;
   									 prev_first_bound = FALSE;
   									 prev_msd_bound = FALSE;
										if (prev_frames_remaining) {
											 prev_bound = TRUE;
											 if (frame) {
															prev_first_bound = TRUE;
											 }
                     	 for (shift = pst->win_l_width[k]; shift <= pst->win_r_width[k]; shift++)
                        	if ( pst->msd_flag[frame + shift] != TRUE) {
                          	prev_msd_bound = TRUE;
                           	break;
                        	}
										 } else {
                     ...
                     for (l = 0; l < pst->vector_length; l++) {
                        m = pst->vector_length * k + l;
												if (prev_bound) {
													if (k == 0) {
                        		pst->sm.mean[msd_frame][m] = sss->sstream[i].prev_par[msd_frame][m];
														pst->sm.ivar[msd_frame][m] = 1.0;
													} else {
														/* improve this by calculating the prev delta, delta-deltas and setting var=0 */
                        		pst->sm.mean[msd_frame][m] = HTS_SStreamSet_get_mean(sss, i, state, m);
														if (prev_msd_bound) { 
															...
															pst->sm.ivar[msd_frame][m] = 0.0;
																...
																/* NR _ to do:  use previous frame is j>0 */
														if (j>0) {
                        			pst->sm.mean[msd_frame][m] = pst->sm.mean[msd_frame-1][m]; 
														} else {
															pst->sm.mean[msd_frame][m] = HTS_SStreamSet_get_mean(sss, i, state, m);
														}
														if (not_bound || k == 0)
															if (j>0) {
                        				pst->sm.ivar[msd_frame][m] = pst->sm.ivar[msd_frame-1][m]; 
															} else {
               ...
               for (k = 0; k < pst->win_size; k++) {
                  ...
                  for (l = 0; l < pst->vector_length; l++) {
                    m = pst->vector_length * k + l;
										if (prev_bound) {
													if (k == 0) {
                        		pst->sm.mean[frame][m] = sss->sstream[i].prev_par[frame][m];
														pst->sm.ivar[frame][m] = 1.0;
													} else {
														/* improve this by calculating the prev delta, delta-deltas and setting var=0 */
                        		pst->sm.mean[frame][m] = 1.0;
														pst->sm.ivar[frame][m] = 0.0;
													}
										 } else {
																/* NR _ to do:  use previous frame is j>0 */
											if (j>0) {
                      			pst->sm.mean[frame][m] = pst->sm.mean[frame-1][m]; 
											} else {
                      		pst->sm.mean[frame][m] = HTS_SStreamSet_get_mean(sss, i, state, m);
											}
                      if (not_bound || k == 0)
													if (j>0) {
                        			pst->sm.ivar[frame][m] = pst->sm.ivar[frame-1][m]; 
													} else {
...
if ( 0) {
     ...
     frame = -1;
		 msd_frame = -1;
		 size_t dbg_frame;
		 size_t mm = sss->sstream[i].vector_length;
		 if (mm > 7) 
					mm = 7;
...
if(frame < 16) {
				dbg_frame = frame;
			  fprintf(stderr,"%ld",frame);				

				if (HTS_SStreamSet_is_msd(sss, i) == TRUE) {
					if (pst->msd_flag[frame] == TRUE)  {
						msd_frame++;
						dbg_frame = msd_frame;
						fprintf(stderr, " (%ld) ", msd_frame);
								for (m=0; m<mm; m++) {
								 fprintf(stderr,"\t%f", pst->par[dbg_frame][m]);				
								}
								fprintf(stderr,"\n");
					} else {
						fprintf(stderr,"   .... non-msd ....\n");
					}
				} else {
								for (m=0; m<mm; m++) {
								 fprintf(stderr,"\t%f", pst->par[dbg_frame][m]);				
